...
class Player77:
	...
	def heuristics(self,last_move):
		...
		for i in range(4):
			cellNumberSelf.append([])
			cellNumberOther.append([])
			for j in range(4):
				cellNumberSelf[i].append(0)
				cellNumberOther[i].append(0)
				...
				if bs[i][j] == '-':
					for k in range(4):
						...
						if CFSRW == 1:
							if CFORW == 0:
								...
								cellNumberSelf[i][j] = max(cellNumberSelf[i][j], CCSRW)
							else:
								heurVal-=cutCell[CCSRW]
						if CFORW == 1:
							if CFSRW == 0:
								heurVal-=contCell[CCORW]
								cellNumberOther[i][j] = max(cellNumberOther[i][j],CCORW)
							else:
						...
						if CFSCL == 1:
							if CFOCL == 0:
								...
								cellNumberSelf[i][j] = max(cellNumberSelf[i][j], CCSCL)
							else:
								heurVal-=cutCell[CCSCL]
						if CFOCL == 1:
							if CFSCL == 0:
								heurVal-=contCell[CCOCL]
								cellNumberOther[i][j] = max(cellNumberOther[i][j],CCOCL)
							else:
					...
					if CFSFD == 1:
						if CFOFD == 0:
							...
							cellNumberSelf[i][j] = max(cellNumberSelf[i][j], CCSFD)
						else:
							heurVal-=cutCell[CCSFD]
					if CFOFD == 1:
						if CFSFD == 0:
							heurVal-=contCell[CCOFD]
							cellNumberOther[i][j] = max(cellNumberOther[i][j],CCOFD)
						else:
					...
					if CFSBD == 1:
						if CFOBD == 0:
							...
							cellNumberSelf[i][j] = max(cellNumberSelf[i][j], CCSBD)
						else:
							heurVal-=cutCell[CCSBD]
					if CFOBD == 1:
						if CFSBD == 0:
							heurVal-=contCell[CCOBD]
							cellNumberOther[i][j] = max(cellNumberOther[i][j],CCOBD)
						else:
		...
		if self.myMove:
			if cellNumberSelf[blockX][blockY] > 1:
			countRow = 0
			countColumn = 0
			countDiagonal = 0
			for i in range(4):
				if cellNumberSelf[i][blockY] > 1:
					countRow += 1
				if cellNumberSelf[blockX][i] > 1:
					countColumn += 1
			
			if blockX == blockY:
				for i in range(4):
					if cellNumberSelf[i][i] > 1:
						countDiagonal += 1
			elif (blockX == 0 and blockY == 3) or (blockX == 1 and blockY == 2)
				or (blockX == 2 and blockY == 1) or (blockX == 3 and blockY == 0)
				for i in range(4):
					if cellNumberSelf[3-i][i] > 1:
						countDiagonal += 1
			
			count = max(countDiagonal, max(countRow, countColumn))
			heurVal += spread[count]
		 
		else:
			if cellNumberOther[blockX][blockY] > 1:
			countRow = 0
			countColumn = 0
			countDiagonal = 0
			for i in range(4):
				if cellNumberOther[i][blockY] > 1:
					countRow += 1
				if cellNumberOther[blockX][i] > 1:
					countColumn += 1
